...
description: Use when creating, editing, or auditing AGENTS.md, CLAUDE.md, system prompts, agent-facing repository instructions, or documents reached through those instructions. Do not use for SKILL.md; use to-skill.
license: LICENSE
---

# Writing for Agents

Make agent-facing instructions predictable while spending only the context their behavior earns. For skill packages and `SKILL.md`, use `to-skill`.

## Audit workflow

1. Map each instruction file, its audience, how it loads, and which branches need it. Done when always-loaded and conditional material are distinguished.
2. Trace every statement to its source of truth. Done when duplicated configuration, commands, and guidance are identified.
3. Rebuild the information hierarchy. Keep universal behavior inline, disclose branch-specific material behind precise pointers, and co-locate each concept's definition, rules, and caveats.
4. Strengthen every action with a checkable, exhaustive completion criterion. Preserve hard guardrails and express the intended behavior positively.
5. Validate every pointer and remove stale, duplicated, or behavior-neutral lines. Done when each remaining line changes agent behavior in a branch that can be named.

## Context pointers

A **context pointer** is a reference held in the agent's context that names some out-of-context material and encodes the condition for reaching it. A line in `AGENTS.md` or `CLAUDE.md` naming another document is a pointer. The pointer's _wording_, not its target, decides when the agent reaches the material, and how reliably. A must-have target behind a weakly worded pointer is a variance bug: sharpen the wording first, and inline the material only if sharpening fails.
...
- **Context load** is the cost of always-loaded material on the agent's window: an `AGENTS.md` or `CLAUDE.md` line, or anything else sitting in context every turn, spending tokens and attention whether or not it fires.
...
Split a run of steps only when visible later steps tempt the agent to rush the current one. Keeping later steps across a real context boundary can drive more legwork; splitting material that every branch needs only adds cognitive load.
